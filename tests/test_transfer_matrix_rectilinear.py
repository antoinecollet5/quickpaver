...
def test_identical_grids_is_identity() -> None:
    ...
    source_grid = quickpaver.RectilinearGrid(
        cx=0.0, cy=0.0, dx=1.0, dy=1.0, nx=3, ny=3, theta=0.0
    )
    target_grid = quickpaver.RectilinearGrid(
        cx=0.0, cy=0.0, dx=1.0, dy=1.0, nx=3, ny=3, theta=0.0
    )
...
def test_separable_coarser_target_conserves_mass() -> None:
    ...
    source_grid = quickpaver.RectilinearGrid(
        cx=0.0, cy=0.0, dx=1.0, dy=1.0, nx=4, ny=4, theta=0.0
    )
    target_grid = quickpaver.RectilinearGrid(
        cx=0.0, cy=0.0, dx=2.0, dy=2.0, nx=2, ny=2, theta=0.0
    )
...
def test_nonseparable_arbitrary_angle_conserves_mass() -> None:
    ...
    source_grid = quickpaver.RectilinearGrid(
        cx=0.0, cy=0.0, dx=0.5, dy=0.5, nx=6, ny=6, theta=0.0
    )
    target_grid = quickpaver.RectilinearGrid(
        cx=0.0, cy=0.0, dx=1.0, dy=1.0, nx=6, ny=6, theta=np.deg2rad(17.0)
    )
...
def test_nonseparable_matches_separable_at_zero_angle() -> None:
    ...
    target_grid_separable = quickpaver.RectilinearGrid(
        cx=0.0, cy=0.0, dx=1.0, dy=1.0, nx=3, ny=3, theta=0.0
    )
    target_grid_nonseparable = quickpaver.RectilinearGrid(
        cx=0.0, cy=0.0, dx=1.0, dy=1.0, nx=3, ny=3, theta=1e-3
    )

    mat_separable = quickpaver.compute_transfer_matrix(
        source_grid,
        target_grid_separable,
        is_sanity_check=True,
    )
    mat_nonseparable = quickpaver.compute_transfer_matrix(
        source_grid,
        target_grid_nonseparable,
        is_sanity_check=True,
    )
    np.testing.assert_allclose(
...
def test_disjoint_grids_have_no_overlap() -> None:
    ...
    target_grid = quickpaver.RectilinearGrid(
        cx=1000.0, cy=1000.0, dx=1.0, dy=1.0, nx=3, ny=3, theta=0.4
    )
...
def test_offset_translation_only_conserves_mass() -> None:
    ...
    source_grid = quickpaver.RectilinearGrid(
        cx=0.0, cy=0.0, dx=1.0, dy=1.0, nx=5, ny=5, theta=0.0
    )
    target_grid = quickpaver.RectilinearGrid(
        cx=0.5, cy=0.0, dx=1.0, dy=1.0, nx=5, ny=5, theta=0.0
    )
